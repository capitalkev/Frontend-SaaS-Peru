...
import { DetailedTable, GroupedTable, BulkActionToolbar } from "./components/Tables"; // IMPORTAMOS LAS TABLAS
...
export function SunatView() {
  ...
  const handleStatusChange = async (ventaId: string, clientId: string, invoiceId: string, newStatus: string) => {
    try {
  ...
  const invoicesFormatted = useMemo(() => ventas.map(v => ({
    ...
    montoNeto: parseFloat(v.monto_neto ?? v.monto_original ?? v.total_cp ?? 0),
    currency: v.moneda,
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <Card className="border-rose-200 bg-rose-50 p-8 text-center max-w-md shadow-sm">
          ...
          <h2 className="text-xl font-bold text-rose-900 mb-2">Error de conexión</h2>
          <p className="text-rose-700 mb-2">{error}</p>
          <p className="text-sm text-rose-600/80 mb-6">Verifica que el servidor de SUNAT esté encendido o que no haya problemas de CORS.</p>
    ...
    <div className="space-y-6 flex flex-col h-[calc(100vh-8rem)]">
      <div>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <ClientFilter clients={clients} selected={selectedClientIds} onChange={ids => { setSelectedClientIds(ids); setCurrentPage(1); }} />
          <CurrencyFilter selected={selectedCurrencies} onChange={curr => { setSelectedCurrencies(curr); setCurrentPage(1); }} />
          {isAdmin && <UserFilter users={users} selected={selectedUserEmails} onChange={emails => { setSelectedUserEmails(emails); setCurrentPage(1); }} />}
        </div>
      </div>

      <div className="shrink-0"><KPIDashboard metrics={metrics} /></div>
      ...
      <Card className="flex-1 border-slate-200 shadow-sm flex flex-col overflow-hidden bg-white min-h-0 relative">
        <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50 shrink-0">
          <div className="flex items-center bg-white p-1 rounded-lg border border-slate-200 shadow-sm">
            <button onClick={() => setViewMode("grouped")} className={`flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === "grouped" ? "bg-brand-50 text-brand-700" : "text-slate-500 hover:text-slate-900"}`}><LayoutGrid className="w-4 h-4" /> Agrupada</button>
            <button onClick={() => setViewMode("detailed")} className={`flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === "detailed" ? "bg-brand-50 text-brand-700" : "text-slate-500 hover:text-slate-900"}`}><List className="w-4 h-4" /> Detallada</button>
          </div>
        ...
        <div className="flex-1 overflow-auto bg-white">
          {loading && ventas.length === 0 ? (
             ...
             viewMode === "grouped" ? (
               <GroupedTable 
                 groups={groupsFormatted} expandedKey={expandedGroupKey} onExpand={setExpandedGroupKey}
                 selectedKeys={selectedInvoiceKeys} 
                 onGroupSelect={(k:string, invKeys:string[], allSelected: boolean) => setSelectedInvoiceKeys(allSelected ? selectedInvoiceKeys.filter(s => !invKeys.includes(s)) : [...new Set([...selectedInvoiceKeys, ...invKeys])])}
                 onInvoiceSelect={(k:string) => setSelectedInvoiceKeys(prev => prev.includes(k) ? prev.filter(x => x !== k) : [...prev, k])}
                 onStatusChange={handleStatusChange}
               />
             ) : (
               <DetailedTable 
                 invoices={invoicesFormatted} selectedKeys={selectedInvoiceKeys}
                 onToggle={(k:string) => setSelectedInvoiceKeys(prev => prev.includes(k) ? prev.filter(x => x !== k) : [...prev, k])}
                 onSelectAll={(chk:boolean) => setSelectedInvoiceKeys(chk ? invoicesFormatted.filter(i=>i.montoNeto!==0).map(i=>i.key) : [])}
                 onStatusChange={handleStatusChange}
               />
             )
          )}
        ...
        <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
          <span className="text-sm font-medium text-slate-500">Página {pagination.page} de {pagination.total_pages} <span className="text-slate-400 font-normal">({pagination.total_items} facturas)</span></span>
        ...
        {selectedInvoiceKeys.length > 0 && (
          <BulkActionToolbar count={selectedInvoiceKeys.length} onClear={() => setSelectedInvoiceKeys([])} onApply={handleBulkUpdate} />
        )}
      </Card>
    </div>
  );

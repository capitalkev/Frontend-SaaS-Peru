...
function BaseSelector({ icon: Icon, label, isOpen, onToggle, children }: any) {
  ...
  return (
    // Redujimos el min-w de 220 a 180px
    <div className="relative min-w-[180px]" ref={ref}>
      {/* Redujimos py-2.5 a py-1.5, px-3 a px-2.5, y text-sm a text-xs */}
      <button
      ...
      {isOpen && (
        // Cambiamos mt-2 a mt-1 y min-w a 200px
...
export function ClientFilter({
  ...
  const label =
    selected.length === 0 || selected.length === clients.length
      ? "Todos los clientes"
      : selected.length === 1
        ? clients.find((c) => c.id === selected[0])?.name
...
export function CurrencyFilter({
  ...
  const [open, setOpen] = useState(false);
  const label =
    selected.length === 0 || selected.length === CURRENCIES.length
      ? "Todas las monedas"
    ...
    >
      <div className="p-1">
        {CURRENCIES.map((c) => (
          <label
            key={c.code}
            className="flex items-center gap-3 p-2.5 hover:bg-slate-50 rounded-lg cursor-pointer"
          >
            <input
              type="checkbox"
...
export function UserFilter({
  ...
  const label =
    selected.length === 0 || selected.length === allUsers.length
      ? "Todos los usuarios"
      : selected.length === 1
        ? allUsers.find((u) => u.email === selected[0])?.nombre
        : `${selected.length} seleccionados`;
...
export function PeriodFilter({ filter, onChange }: any) {
  ...
  const label =
    PRESETS.find((p) => p.key === filter.type)?.label || "Personalizado";
